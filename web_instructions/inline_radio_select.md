...
export const Tooltip = ({ children, text, position = 'top', delay = true, boxAlign = 'center', type = 'default', tip = true, center = true, className }: TooltipProps) => {
  const id = useMemo(() => {
  ...
  return (
    <div>
      ...
      <ReactTooltip
        ...
        opacity={1}
        noArrow={!tip}
        className={clsx('!font-sans !text-[13px] !max-w-52 !rounded-lg', types[type], center ? ' text-center' : ' text-start')}
      >
        {text}
      </ReactTooltip>
    </div>
